...
var (
	...
	ConfigGlobalWsConnectionCycleInterval    = ffc("config.global.ws.connectionCycleInterval", "When non-zero, the client proactively replaces its WebSocket connection on this interval - the new connection is fully established (including post-connect setup) before the old one is quiesced and closed", TimeDurationType)
	ConfigGlobalWsConnectionCycleQuiesceTime = ffc("config.global.ws.connectionCycleQuiesceTime", "The amount of time the old WebSocket connection continues to deliver inbound messages after a connection cycle switches sending to the new connection, before it is closed", TimeDurationType)
